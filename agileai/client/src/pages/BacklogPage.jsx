    ...
    <div className="flex flex-col h-screen bg-background-light dark:bg-background-dark">
          ...
          <div className="flex flex-1 overflow-hidden gap-8">
                  ...
                  {activeSprints.map(sprint => (
                    ...
                    >
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-[10px] text-slate-400 font-bold">{(tasks || []).filter(t => (t.sprint?._id || t.sprint) === sprint._id).length}</span>
                      </div>
                      <h4 className={`text-xs font-bold ${selectedSprintId === sprint._id ? 'text-slate-900' : 'text-slate-600'}`}>{sprint.name}</h4>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col border border-border-light dark:border-border-dark rounded-2xl bg-white dark:bg-card-dark shadow-sm">
              <div className="px-6 py-4 flex items-center justify-between border-b border-border-light dark:border-border-dark sticky top-0 bg-white/80 dark:bg-card-dark/80 backdrop-blur-sm z-10 transition-colors">
                 ...
                 {selectedSprintId !== 'backlog' && activeSprints.find(s => s._id === selectedSprintId)?.status === 'planning' && (
                    <Button size="sm" className="bg-primary hover:bg-primary/90 text-white font-bold text-xs px-4" onClick={() => startSprint(selectedSprintId)}>
                      Start Sprint
                    </Button>
                 )}
              </div>
                ...
                {filteredTasks.length === 0 ? (
                  ...
                  <div className="divide-y divide-border-light dark:divide-border-dark">
                    {filteredTasks.map((task, idx) => (
                      <div key={task._id} className="group px-6 py-4 flex items-center hover:bg-slate-50 dark:hover:bg-white/5 transition-colors cursor-pointer">
                        ...
                        <div className="flex-1 px-4">
                          ...
                          <div className="flex gap-2 mt-1">
                            <span className="text-[9px] bg-slate-100 dark:bg-slate-800 text-slate-500 px-1.5 py-0.5 rounded font-bold uppercase tracking-tight">{task.type}</span>
                        ...
                        <div className="w-32 px-4 flex justify-center items-center">
                          <div className="w-6 h-6 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center text-[10px] font-bold text-slate-600 dark:text-slate-300 ring-2 ring-white dark:ring-surface-dark overflow-hidden">
                            {task.assignee?.avatar ? <img src={task.assignee.avatar} alt="" /> : (task.assignee?.name?.charAt(0) || 'U')}
                          </div>
                        </div>
      ...
      <Modal isOpen={isSprintModalOpen} onClose={() => setIsSprintModalOpen(false)} title="Create Sprint">
        <form onSubmit={handleCreateSprint} className="space-y-4 p-2">
          <Input name="name" label="Sprint Name" placeholder="e.g. Sprint 1" required />
          <div className="grid grid-cols-2 gap-4">
